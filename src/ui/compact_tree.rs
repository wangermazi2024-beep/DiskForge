                ...
                body.rows(ROW_H, flat_rows.len(), |mut row| {
                    ...
                    row.col(|ui| {
                        ...
                        if is_group {
                            ...
                            resp.context_menu(|ui| context_menu(ui, &n.name, &full_path(), n.is_reparse_point(), &fr.abs_path, &action_request));
                        }
                    });
                    row.col(|ui| {
                        let (rect, resp) = paint_bg_and_sense(ui);
                        ...
                        dim_if_hidden(ui, rect);
                        handle_click(&resp);
                        if !is_group { resp.context_menu(|ui| context_menu(ui, &n.name, &full_path(), n.is_reparse_point(), &fr.abs_path, &action_request)); }
                    });
                    row.col(|ui| {
                        let (rect, resp) = paint_bg_and_sense(ui);
                        if !is_group {
...
fn context_menu(ui: &mut egui::Ui, name: &str, full_path: &str, is_reparse: bool, abs_path: &NodePath, action_request: &Cell<Option<TreeAction>>) {
    ...
    let reveal_resp = if is_reparse {
        reveal_resp.on_hover_text("追踪这个符号链接/junction 最终指向的真实文件，可以在资源管理器里定位它")
    } else {
